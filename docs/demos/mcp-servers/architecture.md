...
flowchart TD
    ...
    subgraph cluster ["In-cluster (streamable HTTP, bridged to stdio)"]
        ...
        PS["<b>portal-sandbox</b><br/>read-only · 4 tools"]
        PD["<b>portal-demo</b><br/>read-only · 4 tools"]
    end

    subgraph localreg ["Local, registered by the skill (not in .mcp.json)"]
        AO["<b>ao-sandbox</b><br/>read-only · 32 tools"]
    end
    ...
    CC -->|"static token<br/>(.portal/, gitignored)"| PS
    CC -->|"static token<br/>(.portal/, gitignored)"| PD
    CC -->|"admin password<br/>(scope local)"| AO
...
(`portal-sandbox`). `portal-demo` and `ao-demo` are stated from `.mcp.json`
rather than counted — the `demo` environment has been down since 2026-09-14.
...
| `.aap/` and `.portal/` token files, and the AO and Grafana `claude mcp add` registrations (local only) | Server definitions and access posture |
| Portal static token (`rm -rf .portal/`; regenerated by re-running `playbooks/portal.yml`, not recovered) | `portal-mcp-token` Secret in the cluster |
